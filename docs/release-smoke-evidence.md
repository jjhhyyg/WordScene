...
| DeepSeek live protocol smoke | API | local build host | 1 | PASS | `scripts/run_live_deepseek_translation_smoke.sh` passed at 2026-05-30T03:01:11Z using the ignored local token file. Git commit `d173d8c528a9`. It verified JSON Output with the real DeepSeek API and returned `你好世界` without printing the token. |
