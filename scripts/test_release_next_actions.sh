...
grep -qF '3. Before recording PASS, confirm executable devices with scripts/manual_smoke_environment_preflight.sh.' "$TMPDIR/actions.out"
grep -qF '4. When a physical iPhone or iPad is available, install the iOS candidate with scripts/install_ios_release_candidate.sh before running device smoke.' "$TMPDIR/actions.out"
grep -qF '5. Use scripts/manual_smoke_session_guide.sh to print the preflight, install command, checklist pointer, and environment-scoped record-command templates in one place.' "$TMPDIR/actions.out"
grep -qF '6. Do not call the release complete until scripts/check_release_completion.sh passes.' "$TMPDIR/actions.out"
